/* ============================================================
   VERITAS dApp - genlayer-js integration (Bradbury)
...
const CONFIG = {
  address: "0x70dc688c3DA860Db55c038913af428b162fEF583",
  explorer: "https://explorer-bradbury.genlayer.com/address/0x70dc688c3DA860Db55c038913af428b162fEF583",
};
...
async function connectMetaMask() {
  ...
  state.writeClient = createClient({ chain: testnetBradbury, account: addr, provider: window.ethereum });
  try { await state.writeClient.connect("testnetBradbury"); } catch (e) { console.warn("connect():", e); }
  state.account = addr;
  onConnected();
}
...
async function send(label, functionName, args, value = 0n) {
  ...
  try {
    ...
    await state.readClient.waitForTransactionReceipt({ hash, status: "ACCEPTED" });
    t.close();
